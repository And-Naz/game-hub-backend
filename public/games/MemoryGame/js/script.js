(function (params) {
	...
	let finalImagesArr = [],
		div_class__wrapper = elementFastCreator("<div></div>", 'memory-wrapper'),
		div_class__container = elementFastCreator("<div></div>", 'memory-container'),
		div_class__game_body = elementFastCreator("<div></div>", 'memory-game_body');
	...
	$(document).ready(() => {
		...
		for (var i = 0; i < finalImagesArr.length; i++) {
			let div_class__rectangle = elementFastCreator("<div></div>", 'memory-rectangle')
			...
			setTimeout((e) => { e.addClass('memory-hidden') }, 800, div_class__rectangle)
	...
	function action(elem) {
		...
		if (!($(elem).hasClass("memory-hidden"))) { return }
		// -> Show clicked image
		elem.removeClass('memory-hidden')
		...
		if (rectangleAlreadyClicked) {
			...
			if (elem.data('img') === alreadyClickedElement.data('img')) {
				...
				if (winClicksOfImages.length === origImagesCount) {
					setTimeout(() => {
						$('#Win_poster').toggleClass('memory-hiddenWinPoster')
					}, 100)
				}
			} else {
				...
				secondHideRectangleProcess = setTimeout((e) => {
					e.addClass('memory-hidden')
					alreadyClickedElement.addClass('memory-hidden')
